...
2. validate it received exactly two command-line arguments and that
  ...
  * where `indexFilename` is the pathname of a file that can be read;
  
3.load the index from `indexFilename` into an internal data structure.

4. read search queries from stdin, one per line, until EOF. This includes functionality to: 
 ...
 5. output nothing to stdout other than whta ius indicatged above 
